...
#>
    ...
    Process {
        try {
            $result = Compare-Object -ReferenceObject $updatedConfig.psobject.Properties.name -DifferenceObject $currentConfig.psobject.Properties.name -PassThru -ErrorAction Stop | Where-Object { $_.SideIndicator -eq "<=" } -ErrorAction Stop
        } catch {
            $log4netLogger.error("Failed to compare the two HealOps config files. Failed with > $_")
        }

        # Control that the returned comparison object contains elements
        if (-not $result.Count -ge 1) {
